...
inline T bool_to_int(const bool* data, size_t len) {
	T ret{};
	for (size_t i = 0; i < sizeof(T) * 8 && i < len; ++i) {
		T s{ data[i] };
		s <<= i;
		ret |= s;
	}
...
inline block bool_to_block(const bool* data, size_t len) {
	if (len >= 128)
		return makeBlock(bool_to_int<uint64_t>(data + 64), bool_to_int<uint64_t>(data));
	else if (len >= 64)
		return makeBlock(bool_to_int<uint64_t>(data + 64, len - 64), bool_to_int<uint64_t>(data));
	else
		return makeBlock(bool_to_int<uint64_t>(data + 64, 0), bool_to_int<uint64_t>(data, len));
}

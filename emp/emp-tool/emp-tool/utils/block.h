...
inline void sse_trans(uint8_t *out, uint8_t const *inp, uint64_t nrows,
  ...
  __m512i avx512vec;
  // the below is the same as the following SSE based 16x8 transposition
  // but using AVX512F instructions
  // What doesn't fit here will be caught by the SSE loop and then the leftover processing loop

#include "halfgates_circ_server.h"
...
std::unique_ptr<AESProcessorHalfGateGarbling> HalfGatesCircServerSharing::provideGarblingProcessor() const {
	...
	else
	{
		if (ENABLE_VAES && CPU_FEATURES.vaes && CPU_FEATURES.avx512f && CPU_FEATURES.avx512bw && CPU_FEATURES.avx512vl) {
			if (ENABLE_HYBRID) {
				return std::make_unique<HybridHalfgateGarblingProcessor<InputKeyLTGarblingVaesProcessor, InputKeyLTGarblingAesniProcessor>>(getAndQueue(), m_vGates);
			}
			else {
				return std::make_unique<InputKeyLTGarblingVaesProcessor>(getAndQueue(), m_vGates);
			}
		}
		else if (CPU_FEATURES.aes && CPU_FEATURES.sse4_1)
			return std::make_unique<InputKeyLTGarblingAesniProcessor>(getAndQueue(), m_vGates);
		else
		{
